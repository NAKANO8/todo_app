...
- [x] 1. Foundation: アカウント状態カラムとユーザー情報基盤
...
- [x] 2. Core: 管理者による認証・ユーザー管理のビジネスロジック
...
- [x] 3. Integration: 管理者向けユーザー管理エンドポイントを実装し、アプリケーションに登録する
...
- [x] 4. Integration: 管理者画面（todo-web）
- [x] 4.1 (P) 管理者向けAPIクライアントと型を追加する
...
- [x] 4.2 (P) 管理者以外のアクセスを制限するミドルウェアを拡張する
...
- `role`・`status`のER図・テーブル定義更新は`docs/wiki/Database-Schema.md`（および`.ja.md`）に反映済み（ドキュメント再編後の現行パス）。
