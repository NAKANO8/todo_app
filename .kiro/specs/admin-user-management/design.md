...
- システムに管理者が1人しかいない場合、その管理者は自己降格・自己無効化できない
...
├── repositories/
│   └── auth.repository.ts        # MODIFIED: findAll/updateRole/updateStatus/countActiveAdmins 追加、User型に status 追加
├── services/
...
### Modified Files
- `todo-api/src/repositories/auth.repository.ts` — `User`型に`status: AccountStatus`を追加。`findAll()`、`updateRole(userId, role)`、`updateStatus(userId, status)`、`countActiveAdmins(excludingUserId)`を追加。
- `todo-api/src/services/auth.service.ts` — `login`内、パスワード一致確認後に`user.status === 'disabled'`なら`AppError('account disabled', 403)`を投げる。
...
- `mysql/init.sql` — `users`テーブルに`status`カラムを追加。既存本番DBには別途手動適用が必要（`admin-role` spec同様、デプロイ手順として扱い、タスク一覧には含めない）。
...
sequenceDiagram
    ...
    Svc->>Repo: countActiveAdmins(excluding requesterId) ※自己ターゲット時のみ
    alt 自己ターゲットかつ他に有効な管理者が0人
        Svc-->>Ctrl: AppError 409
        Ctrl-->>Admin: 409 Conflict
    else 続行可能
        Svc->>Repo: updateStatus(targetUserId, "disabled")
...
- 自己降格（ロール変更）も同じ「他に有効な管理者が0人なら拒否」の分岐を`changeRole`側で適用する（Requirement 7.1）。
- 他に管理者が存在する場合、自己ターゲットは他ユーザー対象時と同じ処理を通る（Requirement 7.3）。
...
| 7.1 | 唯一の管理者の自己降格を拒否 | AdminUserService.changeRole, AuthRepository.countActiveAdmins | `PATCH /admin/users/:userId/role` | 無効化フロー（同型分岐） |
| 7.2 | 唯一の管理者の自己無効化を拒否 | AdminUserService.changeStatus, AuthRepository.countActiveAdmins | `PATCH /admin/users/:userId/status` | 無効化フロー |
| 7.3 | 他に管理者がいれば自己操作も通常通り | AdminUserService.changeRole/changeStatus | - | 無効化フロー |
...
| `AdminUserService` | API / Service | 一覧・ロール変更・状態変更・自己保護 | 1.1, 1.2, 2.1, 4.1, 4.2, 5.1, 7.1, 7.2, 7.3 | AuthRepository (P0), SessionService (P0) | Service |
...
- Invariants: `changeRole`/`changeStatus`は、自己ターゲットかつ 変更後に有効な管理者が0人になる場合、`AppError(409)`を投げてDBを変更しない

**Implementation Notes**
- Integration: `countActiveAdmins(excludingUserId)`が0を返す場合のみ自己保護分岐に入る（他ユーザーを対象にした操作には一切影響しない）
- Validation: `newRole`/`newStatus`の値域チェックはルートスキーマ（AJV）側で行う
- Risks: 「最後の管理者」判定は`role=admin AND status=active`のユーザー数（対象本人を除く）に基づく — 無効化済みの管理者は保護対象の母数に含めない
...
- Validation: `userId`はAJVスキーマで`integer`必須（`admin.session.route.ts`と同じ形式）。対象ユーザーが存在しない場合は404（`AuthRepository`側で判定）
- Risks: なし（既存パターンの再利用）
...
- **404 Not Found**: 存在しない`userId`を対象にした操作
- **409 Conflict**: 唯一の管理者による自己降格・自己無効化の試行（Requirement 7）
...
- **Unit Tests**:
  ...
  - `AuthRepository.countActiveAdmins` — `role=admin AND status=active`のみを数え、無効化済み管理者を含めないこと
